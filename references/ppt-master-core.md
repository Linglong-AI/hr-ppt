...
> Embedded in `hr-ppt`: when this reference is loaded by HR-PPT, set
> `${SKILL_DIR}` to the HR-PPT skill root, not to any external `ppt-master`
> folder. All referenced `scripts/`, `templates/`, `references/`, and
> `workflows/` paths are bundled in the HR-PPT repository.
...
2. **Launch + wait for Tier 1.** Background launch; the parent returns when the page writes the tier-1 `result.json`. **Long tool timeout — 600000 ms** (the `--wait` ≈590 s budget):
   ...
   **Codex Desktop launch guard**: do not use bare `--daemon` as a user-facing handoff. Use the `--daemon --wait` command above for the blocking confirmation gate, or run the server in a long-running foreground/session when only demonstrating the page. Verify the reported URL by HTTP request or the in-app browser before telling the user it is open; an unverified PID is not success.
...
- Run it as a long-running side process/session; do not wait for it to exit before generating SVG pages. Do not wait for user confirmation after startup. In Codex Desktop, verify the actual URL loads before reporting it; a detached launch that returns a PID but no reachable page is a failed preview launch.
